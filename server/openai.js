...
import { generateFreePlan, generateFreeChat } from "./gemini.js";
...
export async function chat(messages){
 if(!paidModeEnabled()) return generateFreeChat({messages,instructions:baseSystem});
...
Write the plan in Dutch. Make every scene production-ready for later video generation. Keep the requested dimensions exact.
Return data matching the supplied JSON schema exactly.`;
 const schema=productionSchema();

 if(!paidModeEnabled()){
  const parsed=await generateFreePlan({brief,instructions,schema});
